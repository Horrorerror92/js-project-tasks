...
/*
 ...
 Пример:
   ...
   findAllPSiblings(document.body) // функция должна вернуть массив с элементами div 
   и span т.к. следующим соседом этих элементов является элемент с тегом P
 */
function findAllPSiblings(where) {
    ...
    for (let i = 0; i < childelements.length - 1; i++) {
        if (childelements[i].nextElementSibling.nodeName === 'P') {
            array.push(childelements[i]);
        }
    }
